...
const tokenDmv = require('../../../../../Blockchain/build/contracts/DMVService.json');
...
export class ContractService {
  ...
  // async accountTransactions(accounts){
  //   // const initialvalue = await this.web3js.eth.getStorageAt("0x540bad8ddc8c2cafaf8e81a427d6fdd0c409284f82a44e293b467694049ed20b").then(console.log);
  //   // var contract = require("@truffle/contract"); // acceso a nueva version de libreria

  //   // const paymentContract = contract(tokenAbi);

  //   await this.web3js.eth.Contract.events.allEvents().then(console.log);
 ...
 trasnferEther(originAccount, destinyAccount, amount) {
  ...
  postTransaction(originAccount, data) {
    const that = this;

    return new Promise((resolve, reject) => {
      var contract = require("@truffle/contract"); // acceso a nueva version de libreria
      const paymentContract = contract(tokenDmv);
      paymentContract.setProvider(this.provider);
      paymentContract.deployed().then((instance) => {
        let finalData =  JSON.stringify(data);
        console.log(finalData)
        return instance.postEvent(
          originAccount[0], finalData,
          {
            from: originAccount[0],
          }
          );
